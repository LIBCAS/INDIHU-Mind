...
import core.store.general.DomainStoreImpl;
import core.store.general.GeneralTestEntity;
...
public class DomainStoreTest extends DbTest {
    ...
    @Test
    public void objectPropertyPathTest() {
        GeneralTestEntity s1 = new GeneralTestEntity();
        GeneralTestEntity s2 = new GeneralTestEntity();
        GeneralTestEntity s3 = new GeneralTestEntity();
        GeneralTestEntity other = new GeneralTestEntity();
        store.save(asList(s1, s2, s3, other));
        s2.setRelatedObject(s1);
        s3.setRelatedObject(s1);
        other.setRelatedObject(s2);
        s1.setRelatedObject(other);
        store.save(asList(s1, s2, s3, other));
        List<GeneralTestEntity> byRelation = store.findByRelation(s1.getId());
        assertThat(byRelation, hasSize(2));
        assertThat(byRelation, containsInAnyOrder(s2, s3));
    }
}
