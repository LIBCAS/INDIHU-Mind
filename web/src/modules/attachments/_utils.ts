...
export const createErrorMessage = (err: any, name?: string) => {
  const defaultMessage = "Nastala chyba při přidávání souboru.";
  return err && err.response
    ? err.response.errorType
      ? `${fileErrors[err.response.errorType as FileErrors]}${
          name && err.response.errorType == FileErrors.FILE_TOO_BIG
            ? `: ${name}: ${err.response.errorMessage}`
            : ""
        }`
      : err.response.status === 411 || err.response.status === 403
      ? "Nepodporovaná URL souboru."
      : defaultMessage
    : defaultMessage;
};
