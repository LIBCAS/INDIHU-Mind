...
interface CardDetailContentCardFormProps {
...
export const CardDetailContentCardForm: React.FC<
  CardDetailContentCardFormProps
> = ({ card, setCardContent }) => {
  ...
  const { linkedCards, id } = card.card;
  const isSelected = (id: string) => {
    const index = linkedCards.findIndex((c: any) => c.id === id);
    return index !== -1;
  };
  const onSelect = (cardSelected: any) => {
    const shouldRemove = isSelected(cardSelected.id);
    let newLinkedCards;
    if (shouldRemove) {
      newLinkedCards = linkedCards.filter((c: any) => c.id !== cardSelected.id);
    } else {
      newLinkedCards = [cardSelected, ...linkedCards];
    }
